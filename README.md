...
# create new column :timestamp from :year, :month and :date columns
select(tab, timestamp = (:year, :month, :date) => byrow.((row, _) -> Date(row...))
```
...
| `if_pairs((k,v) -> occursin("id", k) && minimum(x) > 0)` | columns whose names contains "id" and that also have a positive minimum values
...
|----------------------------------|----------------------
| `s => :A`                        | Rename column to `:A` (ignore if `s` doesn't match column)
